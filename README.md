...
To realize the idea we need three loops, the outer i loop for the layer, then the middel k loop for every neuron we need to activate for our output (right sided) operations, and the inner n loop for the input (left sided) neurons and every weight, to calc the products we add to the net variable after we leave the n loop.
